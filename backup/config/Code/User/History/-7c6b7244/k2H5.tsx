...
export default function Home() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // Fetch generated image from internal API
      const imageResponse = await fetch('/api/generate-image');
      const imageData = await imageResponse.json();

      if (!imageResponse.ok) {
        toast.error("Failed to generate artwork image.");
        return;
      }
          ...
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {artworkTiers.map((tier) => (
                <Card
                  key={tier.id}
                  className={`cursor-pointer transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${selectedTier === tier.id
