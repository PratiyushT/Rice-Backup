...
export default function Home() {
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      // 1. Generate ZIP file
      const zipRes = await fetch("/api/generate-artwork-zip", {
        method: "POST",
      ...
      if (!zipRes.ok || !zipData.fileBase64) {
        toast.error("Failed to generate artwork ZIP file.");
        return;
      }
      ...
      const webhookData = {
        ...
        imageFile: {
          url: imageData.imageUrl, // <- direct URL to image
          filename: imageData.imageFilename, // <- readable file name
          alt: imageData.alt,
