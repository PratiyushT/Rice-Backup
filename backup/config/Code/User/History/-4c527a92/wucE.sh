...
# --- Hooks ---
on_tablet_mode() {
    $DEBUG && echo "[hook] Tablet mode ON — starting rotation"
    MONITOR=eDP-1 iio-hyprland eDP-1 &
    echo $! > "$PID_FILE"
}

on_laptop_mode() {
    $DEBUG && echo "[hook] Tablet mode OFF — stopping rotation"
    if [[ -f "$PID_FILE" ]]; then
...
if [[ -z "$EVENT_FILE" || ! -e "$EVENT_FILE" ]]; then
    $DEBUG && echo "[error] Could not find tablet mode switch device."
    exit 1
fi

$DEBUG && echo "[info] Monitoring tablet mode on: $EVENT_FILE"
