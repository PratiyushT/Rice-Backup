...
# ------------ Hooks ------------
start_rotation() {
  # If already running and is iio-hyprland, do nothing
  if [[ -f "$PID_FILE" ]]; then
...
# Return 1 (tablet) or 0 (laptop) if available, else empty
sysfs_state() {
  local p v
  for p in \
...
# Query current state via evtest --query (1 = tablet, 0 = laptop)
evtest_state() {
  local node="$1"
...
# Poll until we get 3 identical reads in a row for a stable initial state
stable_state() {
...
while true; do
  ...
  # Flush stale events quickly
  log "[info] Flushing old events…"
  stdbuf -oL evtest "$EVENT_FILE" | timeout 0.3s cat >/dev/null 2>&1 || true

  # Determine initial state and fire hook once
  ...
  # Live stream; if evtest exits (device reset), loop and re-detect
  stdbuf -oL evtest "$EVENT_FILE" 2>/dev/null | while IFS= read -r line; do
    if [[ "$line" =~ \(SW_TABLET_MODE\),\ value\ ([0-9]+) ]]; then
      NEW="${BASH_REMATCH[1]}"

      # Optional confirm via sysfs to filter spurious edges
      CONFIRM="$(sysfs_state)"
