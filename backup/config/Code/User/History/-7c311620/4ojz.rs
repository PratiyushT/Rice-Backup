...
pub fn sum<'a>(v: Vec<i32>) -> i32 {
    ...
    let vec1 = v[..half].to_vec();
    let vec2 = v[half..].to_vec();
    let left_handle = thread::spawn(|| vec1.into_iter().sum::<i32>());

    let right_handle = thread::spawn(|| {
        vec2.into_iter().sum::<i32>();
    });

    // Join and combine
    let left_sum = left_handle.join().expect("left thread panicked");
    let right_sum = right_handle.join().expect("right thread panicked");
