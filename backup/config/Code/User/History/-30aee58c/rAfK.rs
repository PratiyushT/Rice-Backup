...
impl IndexMut<&TicketId> for TicketStore{
    ...
    self.tickets.iter_mut().find(|val| val.id == *id).unwrap()
    }
}

impl IndexMut<TicketStore> for TicketStore{
    fn index_mut(&mut self, index: TicketStore) -> &mut Self::Output {
        self.tickets.iter_mut().find(|ticket| ticket.id == *index)
    }
}
