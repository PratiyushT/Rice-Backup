...
pub fn sum(slice: &'static [i32]) -> i32 {
    ...
    let left_half = &slice[half..];
    let right_half = &slice[..half];

    let left_spawn = thread::spawn(move || left_half.iter().sum::<i32>());

    let right_spawn = thread::spawn(move || right_half.iter().sum::<i32>());

    let left_handle = left_spawn.join().unwrap();
    let right_handle = right_spawn.join().unwrap();
