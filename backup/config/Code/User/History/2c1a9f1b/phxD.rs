...
impl<'a> IntoIterator for &'a TicketStore{
    ...
    type IntoIter = btree_map::Iter<'a, TicketId, Ticket>;
...
impl FromIterator<&TicketStore> for Vec<TicketId>{
    fn from_iter<T: IntoIterator<Item = TicketStore>>(iter: T) -> Self {
        iter.map();
    }
}
...
mod tests {
    ...
    fn works() {
        ...
        let ids: Vec<TicketId> = (&store).into_iter().map(|t| t).collect();
        let sorted_ids = {
            let mut v = ids.clone();
            v.sort();
            v
