...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<String, Ticket> {
        if title.is_empty() {
        Err(String::from("Title cannot be empty"));
        }
        if title.len() > 50 {
            Err(String::from("Title cannot be longer than 50 bytes"));
        }
        if description.is_empty() {
            Err(String::from("Description cannot be empty"));
        }
        if description.len() > 500 {
            Err(String::from("Description cannot be longer than 500 bytes"));
        }
