...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        if title.is_empty() {
            retErr::<Ticket,String>(String::from("Title cannot be empty"));
        }
        if title.len() > 50 {
            Err::<Ticket,String>(String::from("Title cannot be longer than 50 bytes"));
        }
        if description.is_empty() {
            Err::<Ticket,String>(String::from("Description cannot be empty"));
        }
        if description.len() > 500 {
            Err::<Ticket,String>(String::from("Description cannot be longer than 500 bytes"));
        }
