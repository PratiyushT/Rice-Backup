...
# ===== Immediate-apply helpers for toggles =====
apply_now_if_current_mode() {
  # $1 is "tablet" or "laptop"
  local want="$1"
  local cur=""
  [[ -r "$MODE_FILE" ]] && cur="$(cat "$MODE_FILE" 2>/dev/null || true)"

  # Try linuxflip --print-mode if MODE_FILE is missing
...
# ===== Toggles (rewrite XML, reload Waybar, and apply now if already in that mode) =====
toggle_tablet() {
  read_state
  if [[ "$tablet" == "1" ]]; then
    tablet=0
    if apply_now_if_current_mode "tablet"; then
      stop_if_running
    fi
  else
    tablet=1
...
toggle_laptop() {
  ...
  if [[ "$laptop" == "1" ]]; then
    laptop=0
    if apply_now_if_current_mode "laptop"; then
      stop_if_running
    fi
  else
    laptop=1
