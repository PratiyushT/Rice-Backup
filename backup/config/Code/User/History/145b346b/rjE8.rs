...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    let ticket = Ticket::new(
        title.clone(), description.clone(), status.clone()
    );

    match ticket{
        Err(err) => {
            match err{
                TicketNewError::TitleError(title_err) => panic!("{}", err),
                TicketNewError::DescriptionError(desc_err) =>{
                    return Ticket::new(title, desc_err, status).unwrap()
                }
